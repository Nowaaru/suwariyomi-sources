...
class Tag {
  ...
  constructor(data) {
    if (data === undefined || !("id" in data))
      throw new Error("Attempted to create a tag with invalid data.");
     ...
     * @type {LocalizedString}
     */
    this.localizedDescription = new LocalizedString(
   ...
   */
  get description() {
    if (this.localizedDescription !== undefined)
      return this.localizedDescription.localString;
    return undefined;
  }
  ...
  static async getAllTags() {
    if (Tag.cache.length === 0) {
      ...
      if (res.data.length === 0)
        throw new APIRequestError(
          "The API returned an empty array of tags.",
          APIRequestError.INVALID_RESPONSE
        );
  ...
  static async getTag(indentity) {
    for (let i of await Tag.getAllTags()) {
      if (
        i.id === indentity ||
        i.localizedName.availableLocales.some(
          (elem) => i.localizedName[elem].toLowerCase() === indentity
        )
      ) {
        return i;
      }

...
if (!isBrowser()) {
  registerHeader("User-Agent", `mangadex-full-api/5.9.0 Server-side Node`);
}
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (activeRequestCount >= MAX_REQUESTS_PER_SECOND) {
      await new Promise((resolve) =>
        setTimeout(
          resolve,
          1000 * Math.floor(activeRequestCount / MAX_REQUESTS_PER_SECOND)
        )
      );
    }
    ...
    if (method !== "GET") {
      ...
      else if (requestPayload instanceof Buffer)
        localHeaders[
          "Content-Type"
    ...
    const req = HTTPS.request(
      ...
      (res) => {
        ...
        res.on("end", () => {
          activeRequestCount--;
          if ("set-cookie" in res.headers && !isBrowser())
            registerHeader(
              "cookie",
              res.headers["set-cookie"]
                .concat(requestHeaders["cookie"])
                .join("; ")
            );
          if (
            ...
            try {
              ...
              if (parsedObj === null)
                reject(
                  new APIRequestError(
                    `HTTPS ${method} Response (${endpoint}) returned null`,
                    APIRequestError.INVALID_RESPONSE
                  )
                );
              if (res.statusCode < 400 || res.result === "ok")
                resolve(parsedObj);
            ...
            if (res.statusCode === 429)
              ...
              reject(
                new APIRequestError(
                  `Returned HTML error page ${responsePayload}`,
                  APIRequestError.INVALID_RESPONSE
                )
              );
              ...
              reject(
                new APIRequestError(
                  `Bad/moved endpoint: ${endpoint}`,
                  APIRequestError.INVALID_REQUEST
                )
              );
        ...
        res.on("error", (error) => {
          reject(
            new APIRequestError(
              `HTTPS ${method} Response (${endpoint}) returned an error:\n${error}`
            )
          );
        });
      }
      ...
      reject(
        new APIRequestError(
          `HTTPS ${method} Request (${endpoint}) returned an error:\n${error}`
        )
      );
    });
...
async function apiParameterRequest(baseEndpoint, parameterObject) {
  if (typeof baseEndpoint !== "string" || typeof parameterObject !== "object")
  ...
  for (let [key, value] of Object.entries(parameterObject)) {
    if (value instanceof Array)
      ...
      Object.entries(value).forEach(([k, v]) => params.set(`${key}[${k}]`, v));
    else params.set(key, value);
  }
  let paramsString = params.toString();
  return await apiRequest(
    baseEndpoint + (paramsString.length > 0 ? "?" + paramsString : paramsString)
  );
}
 ...
 * @returns {Promise<Object[]>}
 */
async function apiSearchRequest(
  baseEndpoint,
  parameterObject,
  maxLimit = 100,
  defaultLimit = 10
  ...
  if (initialOffset > MAX_POSSIBLE_LIMIT - Math.min(maxLimit, limit))
    limit = MAX_POSSIBLE_LIMIT - initialOffset;
  if (limit <= 0 || initialOffset >= MAX_POSSIBLE_LIMIT) return [];

  // Need at least one request to find the total items available:
  let initialResponse = await apiParameterRequest(baseEndpoint, {
    ...parameterObject,
    limit: Math.min(limit, maxLimit),
  });
  if (
    !(initialResponse.data instanceof Array) ||
    typeof initialResponse.total !== "number"
  ) {
  ...
  if (
    limit <= maxLimit ||
    initialResponse.total <= initialResponse.data.length + initialOffset
  )
    return initialResponse.data.map((elem) => {
      return { data: elem };
    });
  ...
  let promises = [];
  for (
    let offset = initialOffset + maxLimit;
    offset < limit;
    offset += maxLimit
  ) {
    promises.push(
      apiParameterRequest(baseEndpoint, {
        ...parameterObject,
        limit: Math.min(limit - offset, maxLimit),
        offset: offset,
      })
    );
  }
  ...
  return finalArray.map((elem) => {
    return { data: elem };
  }); // Emulate an array of standard manga objects from the /manga/<id> endpoint
}
 ...
 * @returns {Promise<Object[]>}
 */
async function apiCastedRequest(
  endpoint,
  classObject,
  parameterObject = {},
  maxLimit = 100,
  ...
  let res = await apiSearchRequest(
    endpoint,
    parameterObject,
    maxLimit,
    defaultLimit
...
async function getMultipleIds(
  ...
  let newIds = ids.flat().map((elem) => {
    if (typeof elem === "string") return elem;
      ...
      throw new Error(`Invalid id: ${elem}`);
  ...
  while (newIds.length > 0)
    promises.push(
      searchFunction({ limit: limit, [searchProperty]: newIds.splice(0, 100) })
    );
